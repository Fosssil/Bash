...
# Reading github token through file
...
if [[ -f "$token_file" ]]; then
  ...
  if [[ -z "$git_token" ]]; then
    echo -e "${red}Error: $token_file is empty. Please add your GitHub token to it.${reset}" # When file is empty
    exit 1
  fi
else
  echo -e "${red}Error: $token_file not found. Please create it with your GitHub token.${reset}" # When token file is not available
  exit 1
fi
...
# Unsetting git_token varibale
unset git_token

# Installing nvim lazy packages
printf "\n"
echo -e "${green}Installing packages into neovim...${reset}"
